...
\author{Definitive synthesis of two independent analyses\\
\normalsize (\texttt{discontinuity\_analysis/} and \texttt{reports/issue77/}; each cross-checked the other's numbers)}
\date{July 12, 2026}
...
\#77, and $x_*=960.7687\hPa$, $V_{\max}=74.4149\ms$ for the 1995 case ---
verified independently by both analyses.  The upstream period-2 ``rescue''
...
isolates the discontinuous term \cite{issue77report}:
...
$961.017\leftrightarrow961.671\hPa$ \cite{issue77report}.  An adjacent-day
...
The ballistic (first-connected-lobe) rule was implemented as a
proof-of-mechanism in the companion report \cite{issue77report}: it too
removes the failures, but it answers the free-convection question ---
...
\begin{itemize}
\item \textbf{Failing cases (exact roots, verified independently by both
analyses).}  \#77/Francine: $x_*=950.3136889226\hPa$,
...
committed 2024 ERA5 North-Atlantic sample and its committed outputs.  Two
independent implementations exist and cross-validate:
\begin{itemize}
\item \texttt{discontinuity\_analysis/scripts/} (this document): instrumented
map evaluation (\texttt{gmap.py}), fine scans (\texttt{scan.py}), the
max-work prototype and full-sample validation (\texttt{maxw.py},
\texttt{validate.py}, \texttt{compare.py}, \texttt{dissect.py}), the survey
and SST sweep (\texttt{datagen.py}), the dominance-exchange hunt
(\texttt{humphunt.py}), the reconstruction refutation (\texttt{refine.py}),
and figure generators (\texttt{plots.py}, \texttt{plots2.py}).
\item \texttt{reports/issue77/} \cite{issue77report}: independent
reimplementation (\texttt{analyze.py}, \texttt{cases.py},
\texttt{plot.py}) with one-sided threshold bisection, per-case CSV
evidence, and the ballistic-rule contrast.
\end{itemize}
...
%======================================================================
\section{Provenance of this synthesis}
%======================================================================

This document merges two independently produced analyses that audited each
other.  From \texttt{discontinuity\_analysis/}: the smooth toy models and
the value/maximizer (Berge) demonstration, the 1{,}200-column jump survey
and SST failure-band sweep, the 7{,}093-column repair benchmark and its
fixed-pressure dissection, and the reconstruction-refutation experiment.
From \texttt{reports/issue77/}: the $10^{-9}\hPa$ one-sided threshold table,
the forced-ascent argument that maximum signed work (not ballistic
reachability) is the correct PI semantics, the exact bracketed roots and
the observation that the operational fifth iterate is not the root, the
correct reading of the stopping rule (consecutive iterates), the explicit
IVT hypotheses ($g(400)>400$; residual-checked success), the
1995-case one-sided numbers and control contrast, the staged implementation
plan, and the bibliography.  Where the two analyses computed the same
quantity independently (cycle values, jump location, corrected roots,
$V_{\max}$, $P_{\min}$, benchmark statistics), they agree to the digits
shown.
...
\bibitem{issue77report}
Independent companion study, \emph{Why the Potential-Intensity Pressure
Solver Cycles}, source, scripts, CSV evidence and figures in
\texttt{reports/issue77/}, 12 July 2026.
