...
**This is the definitive writeup**, a synthesis of two independent analyses
that audited and cross-validated each other: this directory and
`reports/issue77/`. `discontinuity_analysis.pdf` (build: `latexmk -pdf`) is a
self-contained mathematical account of the period-2 convergence failure in
the `pcmin` fixed-point iteration, written for a reader with no
atmospheric-physics background.
...
- Independent cross-check: `reports/issue77/` (own scripts, CSV evidence,
  and one-sided threshold bisection)
