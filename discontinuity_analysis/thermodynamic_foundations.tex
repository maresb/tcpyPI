...
follow from it: thermodynamics is the spectral geometry of the Hamiltonian. For
...
---the elementary $e_N(x)$ for fermions, the complete homogeneous $h_N(x)$ for
bosons, and the power-sum leading term $p_1(x)^N/N!$ for the Maxwell--Boltzmann
count \eqref{eq:ZN}---so the indistinguishability that forces the $N!$ is the
$S_N$-symmetry of these functions.
\end{itemize}
...
$(N,\mu)$; ``summing over'' a mechanical variable trades it for its conjugate:
\begin{center}
\small
\begin{tabular}{lll}
\toprule
...
shell is $\rho_{\mathrm{mc}}=\mathbb 1_R/d_R$, and its reduction defines what PSW
...
\varrho_S \;\equiv\; \operatorname{Tr}_E\,\rho_{\mathrm{mc}}
        \;=\; \operatorname{Tr}_E\frac{\mathbb 1_R}{d_R}.
\end{equation}
...
exponentially small in $d_R$. The equal-\emph{a priori} postulate is thereby
...
\big\langle\lVert\rho_S-\varrho_S\rVert_1\big\rangle
 \le\sqrt{\frac{M+1}{\binom{M+n-1}{\,n-1\,}}}
...
and $\rho_S(t)=\rho_S(0)$ is frozen: a deliberately atypical start (all $M$
quanta in oscillator~$1$, $\rho_S=|M\rangle\langle M|$ pure) stays non-thermal
forever. Second, across shells the spectrum is exactly equally spaced, so the
