...
\paragraph{The transform.} Let $\Omega(E)$ be the density of states ($\rho(E)$
of \S\ref{ssec:operator}), so $S(E)=k_B\ln\Omega(E)$ up to a subextensive window
constant. Then $Z$ is exactly its Laplace transform,
\begin{equation}\label{eq:bridge-laplace}
Z(\beta)=\int_0^\infty\!\dd E\,\Omega(E)\,e^{-\beta E}
        =\int_0^\infty\!\dd E\,\exp\!\Big[\tfrac{1}{k_B}S(E)-\beta E\Big],
\end{equation}
invertible by the Bromwich integral
$\Omega(E)=\frac{1}{2\pi i}\int Z(\beta)\,e^{\beta E}\dd\beta$: the two functions
carry identical information. The oscillator shows this concretely. The
...
$\Omega(E)=\operatorname{Tr}\delta(E-\hat H)$ is a comb of spectral spikes, and
...
of states, $\Omega=\Omega_1\!*\Omega_2$, but simply \emph{multiply} their
...
right variable for the solver. Having set $Z$ against its microcanonical shadow,
we return to the object everything hinges on: the single-particle $z$.
